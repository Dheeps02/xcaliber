...
export function AutoScrollToggleDemo() {
  ...
  function handleClick() {
    if (pressing) return
    setPressing(true)
    timerRef.current = setTimeout(() => { setChecked(c => !c); setPressing(false) }, 110)
  }
  ...
  const trackBorder = checked
    ? 'color-mix(in srgb, var(--status-ok) 30%, var(--border))'
    : 'var(--border)'
  ...
  const thumbBorder = checked && !pressing
    ? 'color-mix(in srgb, var(--status-ok) 35%, var(--border))'
    : 'var(--border)'
        ...
        >
          <span
            ...
            style={{
              ...
              backgroundImage: pressing ? 'none' : 'linear-gradient(180deg, var(--shine-4) 0%, var(--shine-1) 50%, var(--shadow-2) 100%)',
